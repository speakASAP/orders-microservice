...
const syntheticDeliveredFixturePath = 'reports/validation/shipment-runtime-readiness/allegro-synthetic-delivered-provider-fixture-current.json';
const syntheticDeliveredFixture = JSON.parse(requireFile(ordersRoot, syntheticDeliveredFixturePath));
const syntheticReturnedFixturePath = 'reports/validation/shipment-runtime-readiness/allegro-synthetic-returned-provider-fixture-current.json';
const syntheticReturnedFixture = JSON.parse(requireFile(ordersRoot, syntheticReturnedFixturePath));
const syntheticIssueFixturePath = 'reports/validation/shipment-runtime-readiness/allegro-synthetic-issue-provider-fixture-current.json';
const syntheticIssueFixture = JSON.parse(requireFile(ordersRoot, syntheticIssueFixturePath));
const broadTrackingScanPath = 'reports/validation/shipment-runtime-readiness/allegro-broad-tracking-scan-2026-07-03.json';
const broadTrackingScan = JSON.parse(requireFile(ordersRoot, broadTrackingScanPath));
...
assert.equal(runtimeGateReport.runtimeEvidence.deployments.warehouse.image, 'localhost:5000/warehouse-microservice:3868df3', 'Warehouse runtime image evidence mismatch');
assert.equal(runtimeGateReport.runtimeEvidence.deployments.allegro.image, 'localhost:5000/allegro-service:c979768', 'Allegro runtime image evidence mismatch');
...
assert.equal(runtimeGateReport.runtimeEvidence.warehouse.correlationReadback.correlations, 3, 'Warehouse correlation readback mismatch');
...
assert.equal(runtimeGateReport.runtimeEvidence.warehouse.providerStatusReadback.observations, 6, 'Warehouse provider observation readback mismatch');
...
assert.equal(runtimeGateReport.runtimeEvidence.warehouse.providerStatusReadback.latestNormalizedWarehouseStatus, 'not_delivered', 'Warehouse normalized status mismatch');
...
assert.equal(runtimeGateReport.runtimeEvidence.warehouse.fulfillmentMutation.fulfillmentStatus, 'not_delivered', 'Warehouse fulfillment status readback mismatch');
assert.equal(runtimeGateReport.runtimeEvidence.orders.warehouseCallbackReadback.centralStatus, 'not_received', 'Orders central status readback mismatch');
...
assert.equal(syntheticDeliveredFixture.schemaVersion, 'orders.allegro_synthetic_delivered_provider_fixture.v1', 'synthetic delivered fixture schema mismatch');
assert.equal(syntheticDeliveredFixture.status, 'synthetic_provider_delivered_lifecycle_proven', 'synthetic delivered fixture status mismatch');
assert.equal(syntheticDeliveredFixture.fixture.latestStatus, 'DELIVERED', 'synthetic fixture must use DELIVERED provider status');
assert.equal(syntheticDeliveredFixture.warehousePost.httpStatus, 201, 'synthetic fixture Warehouse post must succeed');
assert.equal(syntheticDeliveredFixture.warehousePost.statusMutationApplied, true, 'synthetic fixture must apply Warehouse status mutation');
assert.equal(syntheticDeliveredFixture.warehousePost.normalizedWarehouseStatus, 'delivered', 'synthetic fixture normalized Warehouse status mismatch');
assert.equal(syntheticDeliveredFixture.ordersCallbackReadback.resultingLifecycleStage, 'received', 'synthetic fixture Orders lifecycle callback mismatch');
assert.equal(syntheticDeliveredFixture.fixture.rawProviderPayloadDisplayed, false, 'synthetic fixture must not expose provider payload');
assert.equal(syntheticDeliveredFixture.fixture.rawOrderIdsDisplayed, false, 'synthetic fixture must not expose raw order ids');
assert.equal(syntheticReturnedFixture.schemaVersion, 'orders.allegro_synthetic_returned_provider_fixture.v1', 'synthetic returned fixture schema mismatch');
assert.equal(syntheticReturnedFixture.status, 'synthetic_provider_returned_lifecycle_proven', 'synthetic returned fixture status mismatch');
assert.equal(syntheticReturnedFixture.fixture.latestStatus, 'RETURNED', 'synthetic returned fixture must use RETURNED provider status');
assert.equal(syntheticReturnedFixture.warehousePost.httpStatus, 201, 'synthetic returned fixture Warehouse post must succeed');
assert.equal(syntheticReturnedFixture.warehousePost.statusMutationApplied, true, 'synthetic returned fixture must apply Warehouse status mutation');
assert.equal(syntheticReturnedFixture.warehousePost.normalizedWarehouseStatus, 'returned', 'synthetic returned fixture normalized Warehouse status mismatch');
assert.equal(syntheticReturnedFixture.ordersCallbackReadback.resultingLifecycleStage, 'returned', 'synthetic returned fixture Orders lifecycle callback mismatch');
assert.equal(syntheticReturnedFixture.fixture.rawProviderPayloadDisplayed, false, 'synthetic returned fixture must not expose provider payload');
assert.equal(syntheticReturnedFixture.fixture.rawOrderIdsDisplayed, false, 'synthetic returned fixture must not expose raw order ids');
assert.equal(syntheticIssueFixture.schemaVersion, "orders.allegro_synthetic_issue_provider_fixture.v1", "synthetic issue fixture schema mismatch");
assert.equal(syntheticIssueFixture.status, "synthetic_provider_issue_lifecycle_proven", "synthetic issue fixture status mismatch");
assert.equal(syntheticIssueFixture.fixture.latestStatus, "ISSUE", "synthetic issue fixture must use ISSUE provider status");
assert.equal(syntheticIssueFixture.ordersCreateAndFulfillmentSetup.fulfillmentStatusBeforeProviderIssue, "in_delivery", "synthetic issue fixture must start from in_delivery fulfillment");
assert.equal(syntheticIssueFixture.warehousePost.correlationHttpStatus, 201, "synthetic issue fixture correlation registration must succeed");
assert.equal(syntheticIssueFixture.warehousePost.providerStatusHttpStatus, 201, "synthetic issue fixture Warehouse provider post must succeed");
assert.equal(syntheticIssueFixture.warehousePost.statusMutationApplied, true, "synthetic issue fixture must apply Warehouse status mutation");
assert.equal(syntheticIssueFixture.warehousePost.normalizedWarehouseStatus, "not_delivered", "synthetic issue fixture normalized Warehouse status mismatch");
assert.equal(syntheticIssueFixture.ordersCallbackReadback.resultingLifecycleStage, "not_received", "synthetic issue fixture Orders lifecycle callback mismatch");
assert.equal(syntheticIssueFixture.ordersCallbackReadback.customerSawNotReceived, true, "synthetic issue fixture customer lifecycle visibility mismatch");
assert.equal(syntheticIssueFixture.ordersCallbackReadback.adminSawNotReceived, true, "synthetic issue fixture admin lifecycle visibility mismatch");
assert.equal(syntheticIssueFixture.fixture.rawProviderPayloadDisplayed, false, "synthetic issue fixture must not expose provider payload");
assert.equal(syntheticIssueFixture.fixture.rawOrderIdsDisplayed, false, "synthetic issue fixture must not expose raw order ids");
assert.equal(broadTrackingScan.schemaVersion, 'orders.allegro_broad_tracking_scan.v2', 'broad tracking scan schema mismatch');
assert.equal(broadTrackingScan.status, 'blocked_no_non_unknown_provider_sample_now', 'broad tracking scan status mismatch');
assert.equal(broadTrackingScan.providerRead.checkoutFormsScanned, 41, 'broad tracking scan checkout-form count mismatch');
assert.equal(broadTrackingScan.providerRead.trackingEndpoint.http200, 43, 'broad tracking scan tracking read count mismatch');
assert.equal(broadTrackingScan.providerRead.nonUnknownStatusCount, 0, 'broad tracking scan must preserve missing non-UNKNOWN provider sample');
assert.equal(broadTrackingScan.redaction.providerPayloadPrinted, false, 'broad tracking scan must not print provider payloads');
assert.equal(broadTrackingScan.redaction.rawOrderIdsPrinted, false, 'broad tracking scan must not print raw order ids');
...
assert.equal(runtimeGateReport.sourceEvidence.allegroWarehouseServiceRoleHardening.runtimeCutoverBlocked, false, 'runtime cutover must be unblocked after minimal token projection');
...
assertContains(allegroDeployment, 'WAREHOUSE_INTERNAL_SERVICE_TOKEN', 'Allegro deployment must project the Auth-issued Warehouse service token env');
...
const result = {
  ...
  status: 'runtime_proven_source_hardened_minimal_token_projected',
