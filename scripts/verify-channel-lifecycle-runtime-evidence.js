...
const CHANNELS = {
  ...
  allegro: {
    ...
    remainingGates: [
      ...
      'shipment provider runtime correlation and Warehouse -> Orders callback are proven through an approved bounded sanitized smoke; the Auth-issued internal:allegro-service:service token is projected as effective WAREHOUSE_INTERNAL_SERVICE_TOKEN with WAREHOUSE_SERVICE_TOKEN retained only as a source compatibility fallback, and optional gates are real provider live-read plus status-only tracking visibility policy/full-reveal policy',
    ],
  },
  aukro: {
    env: 'AUKRO_REPO_PATH',
...
const result = {
  ...
  remainingIntegrationGates: [
    ...
    'Warehouse/Allegro shipment-status runtime and Orders callback are proven through an approved bounded sanitized smoke; the Auth-issued internal:allegro-service:service token is projected as effective WAREHOUSE_INTERNAL_SERVICE_TOKEN with WAREHOUSE_SERVICE_TOKEN retained only as a source compatibility fallback, and remaining gates are optional real provider live-read plus status-only tracking visibility policy/full-reveal policy',
