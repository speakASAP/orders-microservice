...
for (const marker of ['statusTransitionAudit = transition.approvalAudit', 'hasMatchingStatusIdempotencyKey(order, transition.approvalAudit.idempotencyKey, transition.status)', 'audit?.idempotencyKey === idempotencyKey && audit?.resultingStatus === resultingStatus']) includes(docs.ordersService, marker, 'Orders status/idempotency source');
