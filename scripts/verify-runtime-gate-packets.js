...
const requiredGateMarkers = ['[RESOLVED: W1/W2 live buyer-bound synthetic lifecycle packet executed and verified]','[MISSING: approved buyer/admin bearer/session packets for optional natural human-session/customer-bound proof when product requires proof beyond accepted bounded/service-scoped evidence]','[RESOLVED/NARROWED: FlipFlop W6B central action authority is runtime-complete; future natural human/admin browser proof remains optional/product-gated and does not reopen W6B action authority]','Provider-backed fixture/live smoke evidence is required only if the selected owner option is `provider_backed_supported`','[UNKNOWN: live Bazos marketplace webhook support]','[RESOLVED/NARROWED: Warehouse callback source and approved synthetic customer/admin runtime proof are complete; any extra Warehouse callback smoke beyond W1/W2/W2 is not an autonomous source gap and remains product-approved target/status packet gated]','[MISSING: approved Warehouse fulfillment runtime packet]',paymentProviderDecision,'[MISSING: approved exact payment/refund/provider correction runtime packet naming target order hash/state, payment/provider hashes, actor/approvedBy, approval id, safe reason, idempotency key, sideEffectsHandled payment|warehouse|notification|crm|channel, Warehouse lookup state, channel acknowledgement, and final redacted evidence path]'];
for (const marker of requiredGateMarkers) assertIncludes(contract, marker, 'missing gate marker');
const requiredFields = ['packetId','ownerApproval','scope','actor','target','idempotency','sideEffects','readback','redaction','abortConditions','Auth actor/role mapping','Orders idempotency key and replay policy','sideEffectsHandled.payment|warehouse|notification|crm|channel=true','Required non-secret fields if `provider_backed_supported` is selected','Warehouse-owned warehouseId','Subject-bound ownership policy; email fallback is forbidden'];
...
assertIncludes(contract, 'Guarded create/read/cancel smoke passed with create 201, read 200, cleanup 200', 'W6B runtime-complete evidence in packet contract');
assertIncludes(contract, 'Payment/refund/provider correction remains outside W6B and is governed by the W9 exact-runtime-packet gate', 'W6B payment/provider boundary');
