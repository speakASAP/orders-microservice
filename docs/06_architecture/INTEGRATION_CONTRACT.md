...
| --- | --- | --- | --- | --- | --- | --- |
| Auth | `auth-microservice` | required | JWT identity validated by the role guard on every protected route | `JWT_SECRET`, `AUTH_SERVICE_URL` | Requests without a valid role are rejected (403) | `src/auth/jwt-roles.guard.ts` role checks |
...
| Payments | `payments-microservice` | required | `PUT /:id/payment-status` accepts authenticated terminal-status callbacks | Role `internal:payments-microservice:service`, `PAYMENTS_INTERNAL_SERVICE_TOKEN` | Unauthorized payment-status callbacks are rejected; order retains last known payment status | `src/orders/orders.controller.ts` payment-status route and role guard |
| Catalog | `catalog-microservice` | required | HTTP calls to `CATALOG_SERVICE_URL` for product read and approved pricing writes | `CATALOG_SERVICE_URL=http://catalog-microservice:3200`, `CATALOG_INTERNAL_SERVICE_TOKEN` | Pricing write fails explicitly rather than silently dropping the approved suggestion | `src/pricing/pricing.service.ts` catalog write path |
...
| Warehouse | `warehouse-microservice` | required | `WAREHOUSE_SERVICE_URL` reservation handoff calls with bearer auth | `WAREHOUSE_SERVICE_URL`, `WAREHOUSE_SERVICE_TOKEN`, `WAREHOUSE_RESERVATION_ENABLED` | Reservation handoff is recorded as absent; order creation still succeeds | `src/warehouse/warehouse-reservation.client.ts`, `src/orders/order-fulfillment-handoff.client.ts` |
...
- Every protected route requires a JWT validated against `JWT_SECRET`, enforced by `src/auth/jwt-roles.guard.ts`.
- Internal service callers (`payments-microservice`, `warehouse-microservice`, `invoices-microservice`, marketplace services) are authorized through named internal roles, not shared secrets alone.
- Order status transitions require the `internal:orders-microservice:action-admin` or `global:superadmin` role.
