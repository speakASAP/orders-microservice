...
| W4 Bazos cabinet/status proof | ready now but provider-gated | Bazos UI owner | Prove Bazos buyer/admin lifecycle and note provider webhook gaps | Bazos frontend/shared client/verifier docs | inventing provider webhook contract | `verify:orders-lifecycle-ui`, source/API smoke, provider blocker report | state webhook proof status |
