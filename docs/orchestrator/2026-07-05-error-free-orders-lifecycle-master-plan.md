...
- This is not yet fully proven as production-complete because the central verifier reports `partial_runtime_evidence_natural_buyer_and_provider_smokes_gated`: source contracts and bounded/runtime-core proofs are strong, while natural buyer/provider evidence remains packet-gated. Runtime packet shape is now centralized in docs/orchestrator/2026-07-05-runtime-gate-packet-contracts.md and verified by `npm run verify:runtime-gate-packets`.
