...
- This is not yet fully proven as production-complete because the central verifier reports `partial_runtime_evidence_natural_buyer_and_provider_smokes_gated`: source contracts and bounded/runtime-core proofs are strong, while natural buyer/provider evidence remains packet-gated.
...
- FlipFlop W6 is source-guarded fail-closed for central-owned order lifecycle/payment status mutation and has central lifecycle UI verifier coverage; route-to-Orders admin action wiring and direct live action-admin/customer session smoke remain gated.
...
|---|---|---|---|---|---|---|---|
| W1 Orders runtime proof | source/runtime-core-verified; natural proof gated | Orders validation owner | Prove central create/reserve/pay/fulfillment/status callback with approved non-secret runtime path or keep exact blockers | orders scripts/reports/docs only unless verifier needs tiny source fix | DB mutation without approved packet, secrets, raw PII | focused npm verifiers passed; subject-bound buyer hardening deployed in Orders `ce8c544` | no further autonomous mutation without approved packet |
