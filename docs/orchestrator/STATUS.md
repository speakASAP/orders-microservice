...
Evidence: Allegro runtime is `localhost:5000/allegro-service:d088104` with correlation enabled, default Warehouse URL, and a bearer source present. Warehouse runtime is `localhost:5000/warehouse-microservice:2553452`. Supplemental no-central-order apply replay returned `posted=0`, `disabled=0`, `skipped=1`, reason `MISSING_CENTRAL_ORDER_ID`; Warehouse aggregate counts remained `correlations=1` and `observations=1`. The report preserves the prior bounded callback proof: sanitized replay posted once, Warehouse status `in_delivery`, Orders status `shipped` with payment `paid`.
