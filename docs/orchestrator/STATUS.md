...
IPS: Vision -> Bazos and Orders must share the same product/provider decision boundary; Goal Impact -> W7 now consumes pushed Bazos local W8 intake evidence; System -> Bazos owns local provider decision handoff and Orders verifies aggregation; Feature -> W8 Bazos local intake aggregation; Task -> verify Bazos 3abd0ab and preserve owner-decision blocker; Execution Plan -> Orders docs/verifier-only update, no deploy or live mutation; Coding Prompt -> no provider calls, no secrets/raw IDs/customer/payment/provider/tracking output; Code -> W8 product decision packet verifier and report/status addendum; Validation -> verify:w8-bazos-product-decision-packet plus Bazos verify:bazos-product-decision-intake.
