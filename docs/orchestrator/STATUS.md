## 2026-07-03 - Allegro Warehouse Service Role Source Hardened, Runtime Cutover Blocked
...
Evidence: Warehouse commit `ab7ac6e` requires `internal:allegro-service:service` only on Allegro shipment snapshot/correlation endpoints. Allegro commit `edb3a88` accepts only `WAREHOUSE_SERVICE_TOKEN` or `WAREHOUSE_INTERNAL_SERVICE_TOKEN` for Warehouse shipment correlation and no longer falls back to `ALLEGRO_INTERNAL_SERVICE_TOKEN` or generic `INTERNAL_SERVICE_TOKEN`. Current deployed runtime is still Warehouse `2553452` and Allegro `0cfe401`; the non-mutating auth probe remains recorded as broad Warehouse-admin capable, not minimal Allegro service-role capable.

Remaining gate: `[MISSING: Auth-issued Allegro service token projected to Allegro runtime as WAREHOUSE_SERVICE_TOKEN or WAREHOUSE_INTERNAL_SERVICE_TOKEN before Warehouse/Allegro deploy cutover and recurring provider ingestion]`. Optional gates remain real provider live-read evidence if product requires it and a future audited full-tracking reveal contract.

## 2026-07-03 - Minimal Allegro Service Role Handoff Blocked
...
Required handoff: Auth owner provisions a true `allegro-service` service principal, creates/assigns only `internal:allegro-service:service`, issues the token through Auth-owned runtime code without printing it, and Allegro owner maps that token into the Warehouse correlation token source. Then rerun Orders shipment runtime readiness and no-central-order config smoke.
