2026-07-03 continuation: Allegro minimal Warehouse service-token projection is now runtime-proven. With owner approval, Auth now has an `allegro-service` internal service role/principal path and an Auth-issued token projected through Vault `secret/prod/allegro-service#WAREHOUSE_INTERNAL_SERVICE_TOKEN` into the live Allegro deployment. Allegro source commits b33a155 and d088104 record and deploy the env mapping; live deployment is now localhost:5000/allegro-service:d088104 after the committed env/secret projection deploy. Sanitized runtime proof from the Allegro pod showed the projected token env present, JWT signature valid, `internal:allegro-service:service` present, Warehouse-admin absent, roleCount=1, service type claims present, and no token/identity values printed. A bounded no-central-order replay returned `skipped=1` with `MISSING_CENTRAL_ORDER_ID`; Warehouse aggregate counts stayed unchanged at `correlations=1` and `observations=2`. Persistent changes: Auth role/principal assignment and Vault secret value; both are reversible by service owners. No raw tracking values, provider payloads, customer PII, tokens, raw order ids, raw DB rows, screenshots, or raw DOM were printed. Remaining gates are optional real provider live-read evidence and future audited full-tracking reveal if product/security approves it.
