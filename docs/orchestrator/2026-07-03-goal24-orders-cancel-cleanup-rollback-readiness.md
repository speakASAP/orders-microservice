...
5. Orders-side cancellation cleanup calls Warehouse `cancel` with reason `ORDER_CANCELLED`. If the paid provider smoke has already reached fulfilled/stock-decrement state, a future runtime packet must name whether Warehouse owner expects `cancel`, `return`, or another approved cleanup operation for that exact state. Orders must not invent local stock corrections.
...
- `[MISSING: Warehouse owner-approved cleanup operation for reserved-only, fulfilled/stock-decremented, and partially failed bundle component-line states]`
...
| Warehouse cleanup semantics packet | dependency-gated | Warehouse owner | Define component-line cleanup for reserved, fulfilled, and partial-failure states | Warehouse contract/docs/tests | stock DB edits outside approved workflow | paid-provider target state matrix | `[MISSING]` | Must define whether Orders calls cancel/return/other. |
