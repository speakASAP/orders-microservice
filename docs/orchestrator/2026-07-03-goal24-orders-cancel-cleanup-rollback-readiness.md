...
- Idempotency: the runtime packet must provide an Orders cleanup idempotency key derived from sanitized facts only: approval id, central Orders UUID hash, Payments payment id hash, provider rollback evidence hash, target status, and Warehouse operation matrix version. Orders source now accepts optional `approval.idempotencyKey`, validates it as a sanitized 8-160 character key, and persists it in `orders.statusTransitionAudit` through migration `009_add_order_status_transition_audit.sql`; live cleanup remains `[MISSING: migration/deploy approval for persisted Orders cleanup idempotency key]` until that migration/deploy is approved.
