...
| Bazos | `main` contains worker commit `26af3ae`; current source `adf1a03`; deployed `bazos-service:7365edc` contains `26af3ae` as ancestor. | Source UI verifier passes; unauthenticated `/orders` and `/orders/webhook` return 401; provider-source artifact is superseded by paid replay source evidence with HTTP 200 and zero records/candidates. | Source UI is integrated and paid replay source is deployed, but browser smoke is blocked until live eligible paid multi-product evidence or an approved bounded fixture exists. |
