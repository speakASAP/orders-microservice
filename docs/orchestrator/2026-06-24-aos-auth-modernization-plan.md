# AOS Auth Modernization Plan - orders-microservice

Date: 2026-06-24
Repository: orders-microservice
...
## Vision
Modernize orders-microservice as an Auth consumer that follows the hosted Auth consumer standard while preserving current commerce/backend behavior and repository ownership boundaries.
...
## System
- Consumer service: orders-microservice.
...
## Feature
Adopt the hosted Auth consumer contract for inbound service requests in orders-microservice, including token validation, user/service identity propagation, authorization boundaries, and failure semantics defined by the central standard.

## Task
1. Read the central standard and map its required consumer behaviors onto orders-microservice.
...
Scope: central standard plus orders-microservice docs/source references needed to map required consumer behavior
...
Scope: identify guards, middleware, clients, controllers, tests, and auth-related docs in orders-microservice
